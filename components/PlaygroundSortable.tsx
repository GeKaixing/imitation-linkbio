...
import { ArrowUpRight, Newspaper, Settings, Share, Trash2, Video } from "lucide-react";
...
function SizeSvg({ item }) {
    ...
    return <IsPreview>
        <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 h-7 bg-gray-700 rounded-[5px] flex w-auto items-center px-2 gap-1 absolute -bottom-2 z-30 left-1/2 -translate-x-1/2">
            <div
            ...
            <div
                ...
                className="hover:bg-gray-600 w-5 h-5 flex justify-center items-center rounded-[5px]"
            >
                <Newspaper size={16} color="#F3F4F6"></Newspaper>
            ...
            <div
                ...
                className="hover:bg-gray-600 w-5 h-5 flex justify-center items-center rounded-[5px]"
            >
                <Video size={16} color="#F3F4F6"></Video>
            </div>
            <div
                onPointerDown={(e) => e.stopPropagation()}
                onClick={() => {
                ...
                className="hover:bg-gray-600 w-5 h-5 flex justify-center items-center rounded-[5px]"
            >
                <Settings size={16} color="#F3F4F6"></Settings>
            </div>

        </div>
        <SettingLink open={settingOpen} item={item} onOpenChange={setSettingOpen}></SettingLink>
    </IsPreview>
...
function ItemContent({ item }) {
    ...
    switch (item.type) {
        case "link":
            {
                switch (item.size) {
                            ...
                            <SpinnerSurface loading={loading} submitting={submitting}>
                                <div className="group flex flex-col justify-center items-center rounded-2xl bg-[#23262c]  w-16 h-16 p-2 relative">
                                    <div className=" w-8 h-8">
                                        {item.icon ?
                                            <img height={32} width={32} src={item.icon} alt="logo"></img>
                                            :
                    ...
                    case "little":
                        return (
                            <SpinnerSurface loading={loading} submitting={submitting}>
                                <div className="flex justify-between items-center rounded-2xl bg-[#23262c] w-[426px] h-14 p-2 relative group">
                                    <div className=" w-8 h-8">
                                        {item.icon ?
                                            <img height={32} width={32} src={item.icon} alt="logo"></img>
                                            :
                                    ...
                                    <div className="text-blue-600  text-xs break-all text-center text-nowrap truncate">{item.title ? item.title : item.link}</div>
                    ...
                    case "normal":
                        return (
                            <SpinnerSurface loading={loading} submitting={submitting}>
                                <div className="flex  items-center rounded-2xl  bg-[#23262c] w-[416px] h-[200px] p-2 relative group gap-2">
                                    <div className="flex flex-col w-1/2 flex-1 gap-2  items-start ">
                                        <div className=" w-8 h-8  ">
                                            {item.icon ?
                                                <img height={32} width={32} src={item.icon} alt="logo"></img>
                                                :
                                    ...
                                    <div className="bg-[#252933] rounded-2xl h-[150px] flex-1 flex justify-center items-center"
                                        dangerouslySetInnerHTML={{ __html: item.embedCode }}></div>
                                    <SizeSvg item={item}></SizeSvg>
                                    <DeleteIcon item={item}></DeleteIcon>
                                </div>
                            </SpinnerSurface>
                    ...
                    case "big":
                        return (
                            <SpinnerSurface loading={loading} submitting={submitting}>
                                <div className="flex flex-col  rounded-2xl items-start justify-center gap-2  bg-[#23262c] w-[192px] h-[200px] p-2 relative group">
                                    <div className=" w-8 h-8 ">
                                        {item.icon ?
                                            <img height={32} width={32} src={item.icon} alt="logo"></img>
                                            :
        ...
        case "post":
            return (
                <SpinnerSurface loading={loading} submitting={submitting}>
                    <div className=" group relative flex flex-col justify-center items-center rounded-2xl bg-[#23262c] w-[192px] h-auto p-2 ">
        ...
        case "embed":
            return (
                <SpinnerSurface loading={loading} submitting={submitting}>
                    <div className=" group relative bg-[#23262c]  w-[416px] h-auto p-2  flex flex-col justify-center items-center rounded-2xl">
